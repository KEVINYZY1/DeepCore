...
__forceinline void cuda_kernel_sao( cuda_kernel_t* p, uint32_t mask )
{	
...
__forceinline void cuda_kernel_set_smemnb( cuda_kernel_t* p_kernel, uint32_t nb )
{
    p_kernel->smemnb=nb;
}
__forceinline void cuda_kernel_sgl( cuda_kernel_t* p_kernel, uint32_t gdx, uint32_t gdy, uint32_t gdz )
{
	p_kernel->gdx=gdx; p_kernel->gdy=gdy; p_kernel->gdz=gdz;
}
__forceinline void cuda_kernel_sbl( cuda_kernel_t* p_kernel, uint32_t bdx, uint32_t bdy )
{
...
{
	return cuLaunchKernel( p->id, p->gdx, p->gdy, p->gdz, p->block.x, p->block.y, 1, p->smemnb, s, NULL, (void**)p->extra );
}
