...
int main()
{
	...
	for( int dir=0; dir<2; ++dir )
	{
		for( int e=0; e<sizeof(shapes)/sizeof(shapes[0]); e++ )
		{
			...
			dc_cellconvOp Op;
			size_t auxnb;
			if(dc_create_cellconvOp( &Op, &auxnb, dcMaskPrecisionFloat|dir, 1, pshape, kshape, qshape, 0 )!=dc_success){
			...
			if(dc_cellconv( Op, d_aux, d_c, d_a, d_b, NULL, 1.f, NULL )!=dc_success){
