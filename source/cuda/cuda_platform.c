...
static float __get_device_peak( float n, int cc )
{
    ...
    case 50: 
    case 52: 
    case 61: n_cores_per_SM=128; break;
...
int cuda_platform_init( cuda_platform_t* p_platform )
{
    CUdevice devices[IDC_MAX_DEVICES_PER_NODE], dev;
    char str[IDC_MAX_DEVICES_PER_NODE][64], q[IDC_MAX_DEVICES_PER_NODE];
    int cc[IDC_MAX_DEVICES_PER_NODE], i, k, idev, n, ns, p;
    
    if(cuInit(0)!=CUDA_SUCCESS) return idc_error_invalid_driver;
    n=__get_devices( &devices[0], &cc[0] );
    if(n<=0) return idc_error_invalid_device;
    if(n==1){
        ...
        do{
            ...
            for( i=1, p=0; i<n; ++i ){
                if( idc_strcmp( &str[k][0], &str[q[i]][0] )==0 ){
